...
**问题**
目前用Go实现的这个UIC，没有集成LDAP登录功能，我对这块不熟，社区哪位大侠可以贡献个patch就完美了，没有patch，有个使用LDAP登录校验的例子也行啊

下面介绍从源码编译安装，如果觉得麻烦，可以直接跳到后面使用二进制安装。二进制文件是在64位Linux上编译的，如果你不是64位Linux，那还是要从源码编译安装。
